...
void main(void)
{
    ...
    float window_height = window_params.z;

    float aspect = viewport_width/viewport_height;

    vec2 coords = vec2(position.x,position.y)*0.5 + 0.5;
    coords = coords*vec2((window_width/viewport_width)*aspect, window_height/viewport_height);
    coords = coords*2 - 1;
    coords += vec2((window_xpos/viewport_width)*aspect,window_ypos/viewport_height);
